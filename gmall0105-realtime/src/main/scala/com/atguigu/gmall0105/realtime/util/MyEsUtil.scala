...
object MyEsUtil {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val index: Index = new Index.Builder(Student("zhangsan","lisi")).index("gmall0105_student").`type`("_doc").id("stu123").build()
    jest.execute(index)

  }

  case class Student(name:String,yourname:String)
